...
import torch
from src.wayformer.encoders import EarlyFusionSceneEncoder, Encoder, HierarchicalFusionSceneEncoder, LatentEncoder, LateFusionSceneEncoder, SceneEncoder
...
def test_earlyfusion_scene_encoder_output_shape():
    """
    Test that EarlyFusionSceneEncoder returns the correct output shape.
    """
    num_agents, t_hist, s_i, s_r, s_tls, d_model = 3, 4, 5, 6, 7, 8
    nhead, dim_feedforward, num_layers, num_latents = 2, 16, 2, 4
...
def test_earlyfusion_scene_encoder_with_positional_encodings():
    """
    Test EarlyFusionSceneEncoder with positional encodings.
    """
    num_agents, t_hist, s_i, s_r, s_tls, d_model = 2, 3, 4, 5, 6, 8
    nhead, dim_feedforward, num_layers, num_latents = 2, 16, 2, 4
    encoder = EarlyFusionSceneEncoder(
...
###############################################
# TEST HIERARCHICAL FUSION SCENE ENCODER      #
###############################################

def test_hierfusion_scene_encoder_output_shape():
    """
    Test that HierarchicalFusionSceneEncoder returns the correct output shape.
    """
    num_agents, t_hist, s_i, s_r, s_tls, d_model = 3, 4, 5, 6, 7, 8
    nhead, dim_feedforward, num_layers, num_latents = 2, 16, 2, 4
    encoder = HierarchicalFusionSceneEncoder(
        d_model, nhead, dim_feedforward, num_layers, 0.1, num_latents, "multi_axis"
    )
    agent_histories = torch.randn(num_agents, t_hist, 1, d_model)
    agent_interactions = torch.randn(num_agents, t_hist, s_i, d_model)
...
def test_hierfusion_scene_encoder_with_positional_encodings():
    """
    Test HierarchicalFusionSceneEncoder with positional encodings.
    """
    num_agents, t_hist, s_i, s_r, s_tls, d_model = 2, 3, 4, 5, 6, 8
    nhead, dim_feedforward, num_layers, num_latents = 2, 16, 2, 4
    encoder = HierarchicalFusionSceneEncoder(
...
def test_scene_encoder_earlyfusion_output_shape():
    """
    Test SceneEncoder with early fusion returns correct output shape.
    """
    num_agents, t_hist, s_i, s_r, s_tls, d_model = 3, 4, 5, 6, 7, 8
    nhead, dim_feedforward, num_layers, num_latents = 2, 16, 2, 4
    encoder = SceneEncoder(
        d_model, nhead, dim_feedforward, num_layers, 0.1,
        fusion="early", num_latents=num_latents, attention_type="multi_axis"
    )
    agent_histories = torch.randn(num_agents, t_hist, 1, d_model)
    agent_interactions = torch.randn(num_agents, t_hist, s_i, d_model)
    road_graphs = torch.randn(num_agents, 1, s_r, d_model)
...
def test_scene_encoder_hierfusion_output_shape():
    """
    Test SceneEncoder with hierarchical fusion returns correct output shape.
    """
    num_agents, t_hist, s_i, s_r, s_tls, d_model = 3, 4, 5, 6, 7, 8
    nhead, dim_feedforward, num_layers, num_latents = 2, 16, 2, 4
    encoder = SceneEncoder(
        d_model, nhead, dim_feedforward, num_layers, 0.1,
        fusion="hierarchical", num_latents=num_latents, attention_type="multi_axis"
    )
    agent_histories = torch.randn(num_agents, t_hist, 1, d_model)
    agent_interactions = torch.randn(num_agents, t_hist, s_i, d_model)
    road_graphs = torch.randn(num_agents, 1, s_r, d_model)
